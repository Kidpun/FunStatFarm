...
from .limit import check_limit_message, wait_until_midnight_moscow
...
class FunStatFarm:
    def __init__(self):
        ...
        self.limit_check_bot = getattr(config, "LIMIT_CHECK_BOT", config.TARGET_BOT)
    ...
    async def start(self):
        ...
        has_limit = await self._check_has_limit_messages()
        if has_limit:
            safe_print("[LIMIT] Обнаружено сообщение о лимите. Ожидаю 00:00 МСК...")
            await wait_until_midnight_moscow()
    ...
    async def _check_has_limit_messages(self):
        """Проверяет бота КУДА пересылаем (limit_check_bot, 8345627795) на сообщения о лимите. True если есть."""
        try:
            msgs = await self.client.get_messages(self.limit_check_bot, limit=10)
            for m in msgs or []:
                if m.text and check_limit_message(m.text):
                    safe_print(f"[LIMIT] Найдено сообщение о лимите в боте {self.limit_check_bot} (куда пересылаем)")
                    return True
        except Exception as e:
            safe_print(f"[LIMIT] Ошибка проверки лимита: {e}")
        return False

    async def _recovery_loop(self):
        """После 00:00 МСК: пересылаем 1 сообщение, проверяем лимит. Если лимит — ждём 1 ч и повторяем."""
        from .config import RECOVERY_RETRY_HOURS

        attempt = 0
        while True:
            attempt += 1
            safe_print(f"\n[LIMIT] Recovery попытка #{attempt}: пересылаю 1 сообщение...")
            try:
                ...
                if not messages:
                    ...
                    continue
                msg = None
                for potential_msg in messages:
                    has_media = potential_msg.media is not None
                    has_buttons = potential_msg.buttons is not None and len(potential_msg.buttons) > 0
                    has_text = potential_msg.text is not None and len(potential_msg.text) > 0
                    has_link = has_text and "t.me/" in potential_msg.text
                    if has_link:
                        is_captcha = check_captcha(potential_msg.text, has_media, has_buttons)
                        if not is_captcha:
                            msg = potential_msg
                            break
                if not msg:
                    ...
                    has_buttons = msg.buttons is not None and len(msg.buttons or []) > 0
                    if not msg.text and has_media and has_buttons:
                        safe_print("[LIMIT] Только капча/медиа — отправляю /rand...")
                        await self.client.send_message(self.source_bot, "/rand")
                        await asyncio.sleep(3)
                        continue
                await self.copy_to_target(msg)
                if msg.buttons:
                    for row_idx, row in enumerate(msg.buttons):
                        for btn_idx, btn in enumerate(row):
                            if "change" in (btn.text or "").lower():
                                try:
                                    await msg.click(row_idx, btn_idx)
                                    safe_print("[LIMIT] Кнопка 'change' нажата")
                                except Exception:
                                    pass
                                break
                        else:
                ...
                has_limit = await self._check_has_limit_messages()
                if has_limit:
                    safe_print(f"[LIMIT] Бот всё ещё жалуется на лимит. Жду {RECOVERY_RETRY_HOURS} ч...")
                    await asyncio.sleep(RECOVERY_RETRY_HOURS * 3600)
                    continue
                safe_print("[LIMIT] Лимит сброшен, продолжаю фарм.")
                return
            except Exception as e:
                safe_print(f"[LIMIT] Ошибка recovery: {e}")
    ...
    async def solve_captcha(self, msg):
        ...
        if not word or (not target_emoji and image_data):
            ...
            try:
                ...
                images_to_try = [("raw", image_orig)]
                try:
                    images_to_try.append(("preprocess", preprocess_image(image_orig.copy(), "auto")))
                except Exception:
                    pass
                if OCR_AVAILABLE:
                    for img_src, im in images_to_try:
                        for lang in ["rus", "rus+eng", "eng"]:
                            for psm in [8, 7, 6, 10, 13]:
                                try:
                                    cfg = f"--oem 3 --psm {psm}"
                                    ocr_text_retry = pytesseract.image_to_string(im, lang=lang, config=cfg)
                                    ocr_text_retry = (ocr_text_retry or "").strip()
                                    if not ocr_text_retry or len(ocr_text_retry) < 2:
                                        continue
                                    safe_print(f"[CAPTCHA] Повтор (lang={lang}, psm={psm}, {img_src}): '{ocr_text_retry[:50]}'")
                                    ...
                                    if word_retry and emoji_retry:
                                        safe_print(f"[CAPTCHA] Повторная попытка успешна: '{word_retry}' -> {emoji_retry} ({conf_retry}%)")
                    ...
                    try:
                        ...
                        if results:
                            for result in sorted(results, key=lambda x: x[2] if len(x) > 2 else 0, reverse=True):
                                ...
                                word_e, emoji_e, conf_e = extract_word_from_captcha(easyocr_text, None)
                                if word_e and emoji_e:
                                    safe_print(f"[CAPTCHA] EasyOCR успешно распознал: '{word_e}' -> {emoji_e}")
                                    return word_e, emoji_e, max(conf_e, 70)
                    except Exception as e:
                        safe_print(f"[CAPTCHA] EasyOCR ошибка: {e}")
                
                return word, emoji, confidence
        
        if text:
            text_lower = text.lower()
            menu_words = ['старт', 'start', 'меню', 'menu', 'поиск', 'search', 'скрыться', 'hide']
            if any(w in text_lower for w in menu_words) and not image_data:
                return None, None, 0
    ...
    async def farm_loop(self):
        ...
        while self.running:
            try:
                ...
                try:
                    if await self._check_has_limit_messages():
                        safe_print("\n" + "=" * 60)
                        safe_print("[LIMIT] !!! ДОСТИГНУТ ЛИМИТ (в боте куда пересылаем) !!!")
                        safe_print("=" * 60)
                        safe_print("[LIMIT] Ожидание 00:00 МСК (сброс лимита)...")
                        safe_print("=" * 60)
                        await self.stop_farm()
                ...
                safe_print(f"[MSG] Получено сообщений от {self.source_bot}: {len(messages)}")
                ...
                for potential_msg in messages:
                    ...
                    safe_print(f"[MSG] ID {potential_msg.id}: text={has_text}, link={has_link}, media={has_media}, buttons={has_buttons}")
                    
                    if has_link:
                        is_captcha = check_captcha(potential_msg.text, has_media, has_buttons)
                        safe_print(f"[MSG] ID {potential_msg.id}: captcha={is_captcha}")
                        if not is_captcha:
                            msg = potential_msg
                            safe_print(f"[MSG] ✅ Выбрано сообщение со ссылкой: ID {msg.id}")
                            break
                
                if not msg:
                    msg = messages[0]
                    safe_print(f"[MSG] Используется последнее сообщение: ID {msg.id}")
